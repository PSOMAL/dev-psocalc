...
const BodyImages = ({ id, changeSide }: BodyImagesProps) => {
  const { t } = useTranslation();
  const images = useImages();
  const { areas, setAreas } = useArea();
  const [translationText, setTranslationText] = useState<Record<string, string | string[]>>({});
  const [selectedImages, setSelectedImages] = useState<Record<string, string>>({});
  const isMobile = useDeviceType();

  const selectedArea = areas.find((item) => item.id === id) || { value: 0, redness: 0, thickness: 0, scaling: 0, done: false };
  ...
  const selectedImage = (image: ImageConfig, index: number, imageSrc: string) => {
      ...
      <div className={Styles.bodyTextArea}>
        <div className={Styles.textChangeBodyContainer}>
          <div className={Styles.textChangeBody}>
            ...
            <p className={Styles.handText}>{translationText.handText as string}</p>
          </div>
          {isMobile && (
        ...
        images.map((image) => (
          <div key={image.id} className={Styles.imageSection}>
            <div className={Styles.imageTitle}>
              ...
              <h2>
                {(
                  String(translationText[image.titleKey] || image.titleKey || "")
                ).toUpperCase()}
              </h2>
              <div className={Styles.imageTitleLine}></div>
            </div>
              ...
              {image.src.map((src, index) => (
                <div key={index} className={Styles.imageWrapperContainer}>
                  ...
                  <p className={Styles.imageTitleText}>
                    {(t.translation.imagesTitles as string[])?.[index] || "N/A"}
                  </p>
                </div>
              ))}
            </div>
